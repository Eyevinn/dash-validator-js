...
const DashManifest = function constructor(internalMpd) {
  const self = {
    ...
    get timeAtHead() {
      const lastPeriod = this.mpd.periods[this.mpd.periods.length - 1];
      const as = lastPeriod.adaptationSets.filter(as => as.mimeType == "video/mp4")[0];
      const segments = as.representations[0].segments;
