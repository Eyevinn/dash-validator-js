  ...
  return new Promise((resolve, reject) => {
    function timerFn() {
      mpdUpdateFn().then(() => {
        ...
        if (!hasValidPlayhead(this.mpd, this.headers)) {
          let failed = {};
 ...
 */
function hasValidPlayhead(mpd, headers) {
  let now;
  if (headers && headers["date"]) {
    now = new Date(headers["date"]).getTime();
    //util.log("Reading now from date header", now);
  } else {
    now = Date.now();
    //util.log("Reading now from system clock", now);
  }

  if (Math.abs(mpd.timeAtHead - now) > 18000) {
    //util.log("Diff", mpd.timeAtHead - now);
    return false;
  }
  return true;
}
